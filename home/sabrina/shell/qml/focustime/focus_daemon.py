...
def is_locked():
    for pattern in (['pgrep', '-x', 'swaylock'],
                    ['pgrep', '-f', r'quickshell.*Lock\.qml']):
        try:
            subprocess.check_output(pattern, stderr=subprocess.DEVNULL)
            return True
    ...
    while True:
        try:
            ...
            for line in proc.stdout:
                if 'WindowsChanged' in line or 'FocusedWindow' in line or 'WorkspacesChanged' in line:
